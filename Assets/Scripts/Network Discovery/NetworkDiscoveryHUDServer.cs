...
{
    public sealed class NetworkDiscoveryHUDServer : MonoBehaviour
    {
        [SerializeField]
				...
				if (_endPoints.Count > 0)
				{
					...
					using (new GUILayout.ScrollViewScope(_serversListScrollVector))
					{
						for (int i = 0; i < _endPoints.Count; i++)
						{
							...
							if (GUILayout.Button("Conectar no servidor: " + ipAddress)) {
                                GUILayout.Box("Teste");
                            ...
                            if (GUILayout.Button("Desconectar")) InstanceFinder.ClientManager.StopConnection();
                        }
					}
				}
