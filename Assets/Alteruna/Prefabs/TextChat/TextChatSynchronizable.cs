...
namespace Alteruna
{
	public class TextChatSynchronizable : Synchronizable
	{
		...
		[HideInInspector] public Color NameColor = Color.gray;
		...
		private void Start()
		{
			...
			Multiplayer.RoomJoined.AddListener(OnRoomJoined);
		}

		private void OnRoomJoined(Multiplayer arg0, Room room, User user)
		{
			if (NameColor == Color.gray)
				NameColor = UniqueAvatarColor.HueToId(Color.red, user.Index);
		}
